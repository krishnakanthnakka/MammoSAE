...
class MethodBase:
    ...
    def _get_contribs(self, split):
        ...
        if split == "train":
            labels_train = torch.load(
                os.path.join(probe_labels_dir, f"all_labels_train.pth")
            )
...
class MethodOurs(MethodBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.load_dir = osp.join(
            args.probe_cs_save_dir,
            "..",
            "..",
            sae_config_to_use,
            self.args.probe_dataset,
        )
        ...
        self.decoder_bias = (
            self.concept_layer.post_decoder_bias.bias.detach().cpu().squeeze()
        )
        ...
        if embeddings_path is not None:
            ...
            if type(embeddings_path) == str:
                ...
                self.vocab_txt_all = [
                    np.genfromtxt(vocab_txt_path, dtype=str, delimiter="\n")
                ]
            elif type(embeddings_path) == list:
                self.all_embeddings = [
                    torch.load(e, map_location="cpu") for e in embeddings_path
                ]
                self.vocab_txt_all = [
                    np.genfromtxt(t, dtype=str, delimiter="\n") for t in vocab_txt_path
                ]
        else:
            ...
            self.vocab_txt_all = [
                np.genfromtxt(
                    os.path.join(args.vocab_dir, f"clipdissect_20k.txt"),
                    dtype=str,
                    delimiter="\n",
                )
            ]
    ...
    def get_logits(self):
        ...
        all_logits = torch.load(
            os.path.join(
                self.load_dir,
                self.args.probe_config_name,
                "stats",
                self.args.probe_split,
                "val_all_logits.pt",
            ),
    ...
    def get_concept_name(
        self, concept_idx=None, dic_vec=None, return_sim=False, return_vocab_id=False
    ):
        output = []
        sim = []
        vocab_id = []
        ...
        for vocab_idx, vocab_specific_embedding in enumerate(self.all_embeddings):
            ...
            dic_vec /= dic_vec.norm(dim=0, keepdim=True)
            similarities = torch.matmul(vocab_specific_embedding, dic_vec).squeeze(
                dim=1
            )
    ...
    def get_classifier_weights(self):
        num_concepts = (
            self.args.autoencoder_input_dim_dict[
        ...
        state_dict = torch.load(
            os.path.join(
                ...
                f"on_concepts_{self.args.which_ckpt}_{self.args.probe_config_name}.pt",
            ),
            map_location=self.args.device,
        )
        classifier = torch.nn.Linear(num_concepts, num_classes, bias=False).to(
            self.args.device
        )
    ...
    def get_top_concept_indices_for_class(self, class_idx, num_indices=5):
        if not hasattr(self, "top_indices_for_all_classes"):
            self.top_indices_for_all_classes = self.state_dict["global_stats"][
                self.args.mod_type
            ]["cov"]["node_idxs"][2]
        if num_indices > 10:
            raise ValueError(
    ...
    def _compute_all_concept_embeddings_texts_and_indices(self):
        ...
        for vocab_idx, vocab_specific_embedding in enumerate(self.all_embeddings):
            ...
            similarities = torch.matmul(
                vocab_specific_embedding, self.all_dic_vec
            ).squeeze(dim=1)
            ...
            all_name_similarities = similarities[
                all_selected_indices, np.arange(similarities.shape[1])
            ]
    ...
    def _get_concept_layer(self):
        autoencoder_input_dim = autoencoder_input_dim_dict[
            self.args.ae_input_dim_dict_key["img"]
        ]
