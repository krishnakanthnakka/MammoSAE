...
class MammoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (
            self.args.arch.lower() == "upmc_breast_clip_det_b5_period_n_ft"
            or self.args.arch.lower() == "upmc_vindr_breast_clip_det_b5_period_n_ft"
            or self.args.arch.lower() == "upmc_breast_clip_det_b5_period_n_lp"
            or self.args.arch.lower() == "upmc_vindr_breast_clip_det_b5_period_n_lp"
            or self.args.arch.lower() == "upmc_breast_clip_det_b2_period_n_ft"
            or self.args.arch.lower() == "upmc_vindr_breast_clip_det_b2_period_n_ft"
            or self.args.arch.lower() == "upmc_breast_clip_det_b2_period_n_lp"
            or self.args.arch.lower() == "upmc_vindr_breast_clip_det_b2_period_n_lp"
        ):
        ...
        if self.transform and (
            self.args.arch.lower() == "swin_tiny_custom_norm"
            or self.args.arch.lower() == "swin_base_custom_norm"
        ):
            ...
            img /= img.max()
            img = torch.tensor(
                (img - self.args.mean) / self.args.std, dtype=torch.float32
            )
        else:
            img = np.array(img)
            img = img.astype("float32")
...
class MammoDataset_concept_detection(Dataset):
    ...
    def _generate_image_dict(self):
        ...
        for idx, row in self.annotations.iterrows():
            ...
            labels = [
                label.strip()
                for label in row["finding_categories"].strip("[]").split(",")
            ]
            for label in labels:
                label = label.strip("''")
...
class MammoDataset_concept(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset.lower() == "upmc" and self.target_dataset.lower() == "upmc":
            ...
            img_path = (
                self.dir_path / f"Patient_{study_id}" / self.df.iloc[idx]["IMAGE_ID"]
            )

        elif self.dataset.lower() == "upmc" and self.target_dataset.lower() == "rsna":
            study_id = self.df.iloc[idx]["STUDY_ID"]
            img_path = (
                self.dir_path / str(study_id) / str(self.df.iloc[idx]["IMAGE_ID"])
            )
        ...
        if self.dataset.lower() == "upmc" and self.target_dataset.lower() == "rsna":
            ...
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "clip_v1"
        ):
            y = self.df.iloc[idx]["CLIP_V1"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "mark_v1"
        ):
            y = self.df.iloc[idx]["MARK_V1"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "mole_v1"
        ):
            y = self.df.iloc[idx]["MOLE_V1"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "scar_v1"
        ):
            y = self.df.iloc[idx]["SCAR_V1"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "architectural_distortion"
        ):
            y = self.df.iloc[idx]["Architectural_Distortion"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "asymmetry"
        ):
            y = self.df.iloc[idx]["Asymmetry"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "focal_asymmetry"
        ):
            y = self.df.iloc[idx]["Focal_Asymmetry"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "global_asymmetry"
        ):
            y = self.df.iloc[idx]["Global_Asymmetry"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "mass"
        ):
            y = self.df.iloc[idx]["Mass"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "nipple_retraction"
        ):
            y = self.df.iloc[idx]["Nipple_Retraction"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "skin_retraction"
        ):
            y = self.df.iloc[idx]["Skin_Retraction"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "skin_thickening"
        ):
            y = self.df.iloc[idx]["Skin_Thickening"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "suspicious_calcification"
        ):
            y = self.df.iloc[idx]["Suspicious_Calcification"]
        elif (
            self.args.model_type.lower() == "concept-classifier"
            and self.args.concept.lower() == "suspicious_lymph_node"
        ):
