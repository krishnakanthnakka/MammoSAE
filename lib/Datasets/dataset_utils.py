...
from .dataset_concepts import (
    MammoDataset_concept,
    MammoDataset_concept_detection,
    collater_for_concept_detection,
    MammoDataset,
...
def get_transforms(args):
    if (
        args.dataset.lower() == "rsna" or args.dataset.lower() == "vindr"
        ...
        test_affine_trans = iaa.Sequential(
            [
                iaa.Resize({"height": args.resize, "width": args.resize}),
                iaa.CropToFixedSize(
                    width=args.resize, height=args.resize
...
def get_dataloader_concept_classifier(args, train=True):
    valid_dataset = MammoDataset_concept(
        args=args, df=args.valid_folds, dataset=args.dataset
    )
...
def get_dataloader_concept_detector(args, train=True):
    ...
    valid_loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=False,
        collate_fn=collater_for_concept_detection,
    )
    if train:
...
def get_dataloader_RSNA(args):
    ...
    val_tfm = None
    if (
        args.arch.lower() == "swin_tiny_custom_norm"
        or args.arch.lower() == "swin_base_custom_norm"
    ):
        ...
        val_tfm = torchvision.transforms.Compose(
            [torchvision.transforms.ToTensor(), normalize_transform]
        )
    elif (
        args.arch.lower() == "swin_tiny_custom"
        or args.arch.lower() == "swin_base_custom"
    ):
        train_tfm = Compose(
            [
    ...
    if args.balanced_dataloader == "y":
        weight_path = (
            args.output_path / f"random_sampler_weights_fold{str(args.cur_fold)}.pkl"
        )
        if weight_path.exists():
            weights = pickle.load(open(weight_path, "rb"))
        else:
            weight_for_positive_class = args.sampler_weights[
                f"fold{str(args.cur_fold)}"
            ]["pos_wt"]
            weight_for_negative_class = args.sampler_weights[
                f"fold{str(args.cur_fold)}"
            ]["neg_wt"]
            args.train_folds["weights_random_sampler"] = args.train_folds.apply(
                lambda row: (
                    ...
                    if row["cancer"] == 1
                    else weight_for_negative_class
                ),
                axis=1,
            )
            ...
            pickle.dump(
                ...
                open(
                    args.output_path
                    / f"random_sampler_weights_fold{args.cur_fold}.pkl",
                    "wb",
                ),
            )
