...
def test_forward_pass_result_matches_the_snapshot(
    ...
    model = SparseAutoencoder(
        n_input_features, n_learned_features, n_components=n_components
    )
...
def test_forward_pass_same_without_components_and_1_component() -> None:
    ...
    model_with_1_component = SparseAutoencoder(
        n_input_features, n_learned_features, n_components=1
    )
    ...
    assert torch.allclose(
        output_with_1_component[0].squeeze(1), output_without_components[0]
    )
