...
class LinearEncoder(Module):
    ...
    _weight: Float[
        Parameter,
        Axis.names(
            Axis.COMPONENT_OPTIONAL, Axis.LEARNT_FEATURE, Axis.INPUT_OUTPUT_FEATURE
        ),
    ]
    ...
    def bias(
        self,
    ) -> Float[Parameter, Axis.names(Axis.COMPONENT_OPTIONAL, Axis.LEARNT_FEATURE)]:
        """Bias parameter."""
        return self._bias
    ...
    def __init__(
        ...
        self._weight = Parameter(
            torch.empty(
                shape_with_optional_dimensions(
                    n_components, learnt_features, input_features
                ),
            )
        )
        ...
        with torch.no_grad():
            if component_idx is None:
                if self._n_components is not None:
                    error_message = (
                        "component_idx must be specified when n_components is not None"
                    )
                    raise ValueError(error_message)

                self.weight[dictionary_vector_indices] = updated_dictionary_weights
            else:
                self.weight[component_idx, dictionary_vector_indices] = (
    ...
    def update_bias(
        ...
        with torch.no_grad():
            if component_idx is None:
                ...
                self.bias[component_idx, update_parameter_indices] = (
