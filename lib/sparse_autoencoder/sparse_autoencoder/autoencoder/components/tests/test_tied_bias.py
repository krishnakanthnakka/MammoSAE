...
def test_pre_encoder_subtracts_bias() -> None:
    ...
    encoder_input: Float[Tensor, Axis.names(Axis.BATCH, Axis.INPUT_OUTPUT_FEATURE)] = (
        torch.tensor([[5.0, 3.0, 1.0]])
    )
...
def test_post_encoder_adds_bias() -> None:
    ...
    decoder_output: Float[Tensor, Axis.names(Axis.BATCH, Axis.INPUT_OUTPUT_FEATURE)] = (
        torch.tensor([[5.0, 3.0, 1.0]])
    )
    bias = Parameter(torch.tensor([2.0, 4.0, 6.0]))
