...
def test_forward_pass_same_weights() -> None:
    ...
    _loss, x_reconstruct, acts, _l2_loss, _l1_loss = neel_autoencoder.forward(
        test_batch
    )
...
def test_unit_norm_weights_grad() -> None:
    ...
    autoencoder.decoder._weight.data = decoder_weights  # noqa: SLF001 # type: ignore
    neel_autoencoder.W_dec.data = decoder_weights.T
    autoencoder.decoder._weight.grad = torch.zeros_like(
        autoencoder.decoder.weight
    ...
    assert torch.allclose(
        autoencoder.decoder.weight.grad, neel_autoencoder.W_dec.grad.T, rtol=1e-4
    )
