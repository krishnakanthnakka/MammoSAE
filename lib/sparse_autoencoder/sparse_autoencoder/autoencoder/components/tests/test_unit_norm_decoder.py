...
def test_unit_norm_decreases() -> None:
    ...
    for _ in range(4):
        ...
        layer = UnitNormDecoder(
            learnt_features=3, decoded_features=4, n_components=None
        )
        ...
        weight_norms_without_hook = torch.sum(
            layer_without_hook.weight**2, dim=0
        ).clone()
...
def test_output_same_without_component_dim_vs_with_1_component() -> None:
    ...
    assert torch.allclose(
        output_without_components_dim, output_with_1_component.squeeze(1)
    )
...
def test_update_dictionary_vectors_with_neurons(
    ...
    decoder.update_dictionary_vectors(
        dictionary_vector_indices, updates, component_idx=0
    )
