...
def replace_activations_hook(
    ...
    squashed_value: Float[Tensor, Axis.names(Axis.BATCH, Axis.INPUT_OUTPUT_FEATURE)] = (
        value.view(-1, value.size(-1))
    )
    ...
    _learned_activations, output_activations = sparse_autoencoder.forward(
        squashed_value
    )
