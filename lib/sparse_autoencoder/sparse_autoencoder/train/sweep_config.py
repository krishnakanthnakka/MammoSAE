...
DEFAULT_BATCH_SIZE: int = (
    8192  # Should be a multiple of source batch size and context size
)
...
class ActivationResamplerHyperparameters(NestedParameter):
    ...
    resample_dataset_size: Parameter[int] = field(
        default=Parameter(DEFAULT_BATCH_SIZE * 100)
    )
...
class SourceDataHyperparameters(NestedParameter):
    ...
    dataset_column_name: Parameter[str] | None = field(
        default=Parameter(value="input_ids")
    )
    """Dataset column name."""
    ...
    def __post_init__(self) -> None:
        ...
        """
        if self.pre_tokenized.value is False and not isinstance(
            self.tokenizer_name, Parameter
        ):
            error_message = (
                "The tokenizer name must be specified, when `pre_tokenized` is False."
            )
            raise ValueError(error_message)

        if self.pre_tokenized.value is True and isinstance(
            self.tokenizer_name, Parameter
        ):
            error_message = (
                "The tokenizer name must not be set, when `pre_tokenized` is True."
            )
            raise ValueError(error_message)
...
class PipelineHyperparameters(NestedParameter):
    ...
    source_data_batch_size: Parameter[int] = field(
        default=Parameter(DEFAULT_SOURCE_BATCH_SIZE)
    )
...
class Hyperparameters(Parameters):
    ...
    autoencoder: AutoencoderHyperparameters = field(
        default=AutoencoderHyperparameters()
    )
