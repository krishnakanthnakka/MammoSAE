...
class TestInit:
    ...
    def test_source_data_iterator_stateful(self, pipeline_fixture: Pipeline) -> None:
        ...
        assert not torch.allclose(
            sample1, sample2
        ), "Source data iterator should be stateful."
...
class TestGenerateActivations:
    ...
    def test_store_has_unique_items(self, pipeline_fixture: Pipeline) -> None:
        ...
        assert (
            len(unique_activations) >= expected_min_length
        ), "Store items should be unique"

    @pytest.mark.integration_test()
    def test_two_runs_generate_different_activations(
        self, pipeline_fixture: Pipeline
    ) -> None:
...
class TestUpdateParameters:
    ...
    def test_weights_biases_changed(self, pipeline_fixture: Pipeline) -> None:
        ...
        encoder_weight_before = (
            pipeline_fixture.autoencoder.encoder.weight.clone().detach()
        )
        encoder_bias_before = pipeline_fixture.autoencoder.encoder.bias.clone().detach()
        decoder_weight_before = (
            pipeline_fixture.autoencoder.decoder.weight.clone().detach()
        )
        ...
        pipeline_fixture.update_parameters(
            [
                ParameterUpdateResults(
                    dead_neuron_indices=dead_neuron_indices,
                    dead_encoder_weight_updates=torch.zeros_like(
                        pipeline_fixture.autoencoder.encoder.weight[
                            0, dead_neuron_indices
                        ],
                        dtype=torch.float,
                    ),
                    dead_encoder_bias_updates=torch.zeros_like(
                        pipeline_fixture.autoencoder.encoder.bias[
                            0, dead_neuron_indices
                        ],
                        dtype=torch.float,
                    ),
                    dead_decoder_weight_updates=torch.zeros_like(
                        pipeline_fixture.autoencoder.decoder.weight[
                            0, :, dead_neuron_indices
                        ],
                        dtype=torch.float,
                    ),
...
class TestRunPipeline:
    ...
    def test_run_pipeline_calls_all_methods(self, pipeline_fixture: Pipeline) -> None:
        ...
        assert (
            pipeline_fixture.activation_resampler.step_resampler.call_count
            == total_loops
