...
from sparse_autoencoder.metrics.metrics_container import (
    MetricsContainer,
    default_metrics,
)
...
class Pipeline:
    ...
    def generate_activations(self, store_size: PositiveInt) -> TensorActivationStore:
        ...
        store = TensorActivationStore(
            store_size, n_neurons, n_components=self.n_components
        )
        ...
        for component_idx, cache_name in enumerate(self.cache_names):
            hook = partial(
                store_activations_hook, store=store, component_idx=component_idx
            )
        ...
        with torch.no_grad():
            while len(store) < store_size:
                ...
                ] = batch["input_ids"].to(source_model_device)
    ...
    def train_autoencoder(
        ...
        for store_batch in activations_dataloader:
            ...
            if (
                ...
                and int(self.total_activations_trained_on / train_batch_size)
                % self.log_frequency
                == 0
            ):
                log = {}
    ...
    def update_parameters(
        self, parameter_updates: list[ParameterUpdateResults]
    ) -> None:
        ...
        for component_idx, cache_name in enumerate(self.cache_names):
            for batch_idx in range(losses.shape[0]):
                ...
                with torch.no_grad():
                    ...
                    losses_with_reconstruction[batch_idx, component_idx] = (
                        loss_with_reconstruction.sum()
                    )
                    losses_with_zero_ablation[batch_idx, component_idx] = (
    ...
    def save_checkpoint(self, *, is_final: bool = False) -> Path:
        ...
        # Create the name
        name: str = (
            f"{self.run_name}_{'final' if is_final else self.total_activations_trained_on}"
        )
    ...
    def run_pipeline(
        ...
        with tqdm(
            ...
            for _ in range(0, max_activations, store_size):
                ...
                activation_store: TensorActivationStore = self.generate_activations(
                    store_size
                )
                ...
                if (
                    validate_frequency is not None
                    and last_validated >= validate_frequency
                ):
                ...
                if (
                    ...
                    and last_checkpoint >= checkpoint_frequency
                ):
                    last_checkpoint = 0
