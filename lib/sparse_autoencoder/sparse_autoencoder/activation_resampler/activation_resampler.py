...
class LossInputActivationsTuple(NamedTuple):
    ...
    input_activations: Float[
        Tensor,
        Axis.names(Axis.BATCH, Axis.COMPONENT_OPTIONAL, Axis.INPUT_OUTPUT_FEATURE),
    ]
...
class ActivationResampler(AbstractActivationResampler):
    ...
    _collated_neuron_activity: Float[
    ...
    def __init__(
        ...
        threshold_is_dead_portion_fires: Annotated[
            float, Field(strict=True, ge=0, le=1)
        ] = 0.0,
        ...
        if n_activations_activity_collate > resample_interval:
            error_message = "Number of steps to collate must be less than or equal to the resample interval."
            raise ValueError(error_message)

        super().__init__()
        self.neuron_activity_window_end = resample_interval
        self.neuron_activity_window_start = (
            resample_interval - n_activations_activity_collate
        )
    ...
    def compute_loss_and_get_activations(
        ...
        with torch.no_grad():
            ...
            dataloader = DataLoader(store, batch_size=train_batch_size, shuffle=True)
    ...
    def sample_input(
        ...
        for component_idx, component_n_samples in enumerate(n_samples):
            component_probabilities: Float[Tensor, Axis.BATCH] = (
                get_component_slice_tensor(
            ...
            component_sample_indices: Int64[Tensor, Axis.LEARNT_FEATURE_IDX] = (
                torch.multinomial(
    ...
    def renormalize_and_scale(
        ...
        average_alive_norm: Float[Tensor, Axis.SINGLE_ITEM] = (
            alive_encoder_weights.norm(dim=-1).mean()
        )
        ...
        with torch.no_grad():
            ...
            for component_idx in range(self._n_components):
                ...
                dead_decoder_weight_updates = rearrange(
                    renormalized_input,
                    "dead_neuron input_feature -> input_feature dead_neuron",
                )
                ...
                encoder_weight: Float[
                    ...
                    autoencoder.encoder.weight, 3, 0, component_idx
                )
    ...
    def step_resampler(
        ...
        if self._n_times_resampled < self._max_n_resamples:
            ...
            if (
                self._activations_seen_since_last_resample
                >= self.neuron_activity_window_start
            ):
            ...
            if (
                self._activations_seen_since_last_resample
                >= self.neuron_activity_window_end
            ) and (self.epoch_since_last_resample >= self.resample_epoch_freq):
