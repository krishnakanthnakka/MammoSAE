...
def find_metric_result(
    ...
    *,
    ...
    if len(results) == 0:
        metric_names = ",\n ".join(
            [f"{metric.name} {metric.postfix or ''}" for metric in metrics]
        )
        error_message = (
            f"Metric not found. The only metrics found were:\n {metric_names}"
        )
        raise ValueError(error_message)
    ...
    error_message = (
        f"Multiple metrics found: name={name}, postfix={postfix}, location={location}"
    )
    raise ValueError(error_message)
