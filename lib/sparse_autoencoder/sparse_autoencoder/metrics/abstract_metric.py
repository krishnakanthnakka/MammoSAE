...
class MetricResult:
    ...
    component_wise_values: (
        Sequence[WandbSupportedLogTypes]
    ...
    def __init__(
        ...
        location: MetricLocation,
        aggregate_approach: (
            ComponentAggregationApproach | None
        ) = ComponentAggregationApproach.ALL,
    ...
    def aggregate_value(  # noqa: PLR0911
        ...
        if (
            isinstance(self.component_wise_values, (Sequence, list, tuple, set))
        ) and all(isinstance(x, (int, float)) for x in self.component_wise_values):
    ...
    def wandb_log(self) -> dict[str, WandbSupportedLogTypes]:
        ...
        if self.n_components > 1:
            for component_name, value in zip(
                self._component_names, self.component_wise_values
            ):
                component_wise_logs[
                    self.create_wandb_name(component_name=component_name)
                ] = value
            ...
            aggregate_log = {
                self.create_wandb_name(
                    aggregation_approach=(
                        self.aggregate_approach if self.n_components > 1 else None
                    )
