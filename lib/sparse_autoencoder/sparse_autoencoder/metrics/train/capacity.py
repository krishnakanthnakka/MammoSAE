...
class CapacityMetric(AbstractTrainMetric):
    ...
    def capacities(
        ...
        squared_dot_products: Float[
            Tensor, Axis.names(Axis.BATCH, Axis.BATCH, Axis.COMPONENT)
        ] = (
            einops.einsum(
                features,
                features,
        ...
        sum_of_sq_dot: Float[Tensor, Axis.names(Axis.COMPONENT, Axis.BATCH)] = (
            squared_dot_products.sum(dim=-1)
        )

        diagonal: Float[Tensor, Axis.names(Axis.COMPONENT, Axis.BATCH)] = (
            torch.diagonal(squared_dot_products, dim1=1, dim2=2)
        )
    ...
    def wandb_capacities_histogram(
        ...
        """
        np_capacities: Float[np.ndarray, Axis.names(Axis.COMPONENT, Axis.BATCH)] = (
            capacities.cpu().numpy()
        )

        np_histograms = [
            histogram(capacity, bins=20, range=(0, 1)) for capacity in np_capacities
        ]

        return [
            wandb.Histogram(np_histogram=np_histogram) for np_histogram in np_histograms
        ]
