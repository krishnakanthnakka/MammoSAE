...
def test_adds_component_dimension() -> None:
    ...
    assert metric_data.decoded_activations.shape == (
        d_batch,
        1,
        n_input_output_features,
    )
...
def test_no_changes_with_component_dimension_already_added() -> None:
    ...
    assert metric_data.input_activations.shape == (
        d_batch,
        n_components,
        n_input_output_features,
    )
    assert metric_data.learned_activations.shape == (
        d_batch,
        n_components,
        n_learned_features,
    )
