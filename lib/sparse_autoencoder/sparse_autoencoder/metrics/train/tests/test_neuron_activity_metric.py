...
class TestNeuronActivityHorizonData:
    ...
    def test_results_match_expectations(self) -> None:
        ...
        dead_over_30_activations = find_metric_result(
            res, postfix="dead_over_30_activations"
        )
        assert dead_over_30_activations.component_wise_values[0] == expected_dead

        alive_over_30_activations = find_metric_result(
            res, postfix="alive_over_30_activations"
        )
        ...
        assert (
            almost_dead_over_30_activations.component_wise_values[0]
            == expected_almost_dead
        )
...
class TestNeuronActivityMetric:
    ...
    def test_dead_neuron_count(
        ...
        dead_over_1_activations = find_metric_result(
            metrics, postfix="dead_over_1_activations"
        )
        alive_over_1_activations = find_metric_result(
            metrics, postfix="alive_over_1_activations"
        )
        ...
        assert torch.allclose(
            dead_over_1_activations.component_wise_values, expected_dead_count
        )
        assert torch.allclose(
            alive_over_1_activations.component_wise_values, expected_alive_count
        )

    def test_alive_neuron_count(
        self, sample_neuron_activity_data: TrainMetricData
    ) -> None:
        ...
        assert (
            alive_over_1_activations.component_wise_values[0]
            == expected_alive_neuron_count
        )
