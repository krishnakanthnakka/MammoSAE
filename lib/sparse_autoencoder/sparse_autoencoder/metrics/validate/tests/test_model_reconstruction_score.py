...
from sparse_autoencoder.metrics.validate.model_reconstruction_score import (
    ModelReconstructionScore,
)
    ...
    [
        (
            ValidationMetricData(
                ...
                source_model_loss_with_reconstruction=torch.tensor(
                    [[3.0], [3.0], [3.0]]
                ),
                source_model_loss_with_zero_ablation=torch.tensor(
                    [[4.0], [4.0], [4.0]]
                ),
            ),
            1.0,
...
def test_model_reconstruction_score_various_data(
    ...
    reconstruction_score = find_metric_result(
        calculated, name="reconstruction_score", postfix=None
    )
