...
def eval(args, alpha, latent_neurons):
    ...
    def hook_fn(module, input, output):
        ...
        # set the latent neurons position to alpha ie, 0.0
        if latent_neurons != []:
            concepts[:, :, latent_neurons] = alpha
...
if __name__ == "__main__":
    ...
    outpath = (
        "./results/topk_off_intervention"  # args.save_dir_sae_ckpts[args.modality]
    )
    ...
    if not os.path.exists(
        os.path.join(
            outpath, f"topk_off_intervention_{args.label}_{args.checkpoint_mode}"
        )
    ):
        os.makedirs(
            os.path.join(
    ...
    for k in topks:
        ...
        print(
            f"Conducting topk_off intervention for latent neurons: {latent_neurons} "
        ...
        with open(
            os.path.join(
                outpath,
                f"topk_off_intervention_{args.label}_{args.checkpoint_mode}",
