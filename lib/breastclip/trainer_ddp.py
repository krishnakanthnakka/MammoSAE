...
def run_ddp(local_rank, cfg: Dict):
    ...
    if distributed:
        ...
        ), f"world_size cannot be greater than available CUDA devices"

        # Set the CUDA device based on local_rank
        device = torch.device(
            f"cuda:{local_rank}"
        )  # Define the device for model and tensors

        # Set the CUDA device for the current process
        torch.cuda.set_device(
            device
        )  # It tells PyTorch which GPU this particular process should primarily use
    ...
    if "args_path" in cfg["base"]["output"]:
        ...
        with open(
            os.path.join(cfg["base"]["output"]["args_path"], "cfg.pkl"), "wb"
        ) as f:
            pickle.dump(cfg, f)
    ...
    if cfg["model"]["image_encoder"]["model_type"] == "swin":
        ...
        cfg["model"]["image_encoder"]["name"] == "resnet101"
        or cfg["model"]["image_encoder"]["name"] == "resnet152"
        or cfg["model"]["image_encoder"]["name"] == "tf_efficientnet_b5_ns-detect"
        or cfg["model"]["image_encoder"]["name"] == "tf_efficientnetv2-detect"
    ):
    ...
    train_dataloader, train_sampler = datamodule.train_dataloader(
    ...
    if "total_epochs" in cfg["scheduler"]["config"]:
        cfg["scheduler"]["config"]["total_steps"] = (
            len(train_dataloader) * cfg["scheduler"]["config"]["total_epochs"]
        )
    if "warmup_epochs" in cfg["scheduler"]["config"]:
        if isinstance(cfg["scheduler"]["config"]["warmup_epochs"], int):
            cfg["scheduler"]["config"]["warmup_steps"] = (
                len(train_dataloader) * cfg["scheduler"]["config"]["warmup_epochs"]
            )
        elif isinstance(cfg["scheduler"]["config"]["warmup_epochs"], float):
            cfg["scheduler"]["config"]["warmup_steps"] = cfg["scheduler"]["config"][
                "warmup_epochs"
            ]
    ...
    try:
        for epoch in range(epoch_resume, total_epochs):
            ...
            for data_name, loss_dict in val_loss_dict_per_dataset.items():
                for loss_key, v in loss_dict.items():
                    util.GlobalEnv.get().summary_writer.valid.add_scalar(
                        f"loss_per_epoch/{loss_key}/{data_name}", v, epoch + 1
                    )
            ...
            for loss_key in avg_val_loss_per_loss:
                ...
                util.GlobalEnv.get().summary_writer.valid.add_scalar(
                    f"loss_per_epoch/{loss_key}",
                    avg_val_loss_per_loss[loss_key],
                    epoch + 1,
                )
...
def train(
    ...
    if util.GlobalEnv.get().local_rank < 1:
        progress_iter = tqdm(
            ...
            desc=f"[{epoch:03d}/{total_epochs:03d} epoch train]",
    ...
    for idx, batch in progress_iter:
        ...
        if (
            image_encoder.lower() == "swin"
            or image_encoder.lower() == "resnet101"
            or image_encoder.lower() == "resnet152"
            or image_encoder.lower() == "tf_efficientnet_b5_ns-detect"
            or image_encoder.lower() == "tf_efficientnetv2-detect"
        ):
            if "images" in batch:
                batch["images"] = batch["images"].squeeze(1).permute(0, 3, 1, 2)
            if "image_views" in batch:
                batch["image_views"] = (
                    batch["image_views"].squeeze(1).permute(0, 3, 1, 2)
                )
        ...
        if idx % print_step == 0 and util.GlobalEnv.get().local_rank < 1:
            for k, lr in enumerate(scheduler.get_last_lr()):
                util.GlobalEnv.get().summary_writer.train.add_scalar(
                    f"hyperparam/lr-{k}", lr, scheduler._step_count
                )
            util.GlobalEnv.get().summary_writer.train.add_scalar(
                "loss", total_loss, scheduler._step_count
            )

            for k in loss_dict:
                util.GlobalEnv.get().summary_writer.train.add_scalar(
                    f"loss/{k}", loss_dict[k], scheduler._step_count
                )
    ...
    with torch.no_grad():
        for data_name, dataloader in dataloader_dict.items():
            ...
            if util.GlobalEnv.get().local_rank < 1:
                progress_iter = tqdm(
                    enumerate(dataloader),
                    desc=f"[{epoch:03d}/{total_epochs:03d} epoch valid {data_name}]",
                    total=len(dataloader),
                )
            else:
                progress_iter = enumerate(dataloader)
            ...
            for idx, batch in progress_iter:
                ...
                if util.GlobalEnv.get().world_size > 1:
                    for loss_key in loss_dict:
                        ...
                        loss_dict[loss_key] = (
                            loss_dict[loss_key] / util.GlobalEnv.get().world_size
                        )
                ...
                if (
                    idx % print_step == 0 or idx == len(dataloader) - 1
                ) and local_rank < 1:
                    progress_iter.set_postfix(
                        {
