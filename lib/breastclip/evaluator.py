...
class Evaluator:
    def __init__(self, config: Dict, ckpt_path):
        ...
        if self.ckpt_config["model"]["image_encoder"]["model_type"] == "swin":
            self.clip_image_encoder = self.ckpt_config["model"]["image_encoder"][
                "model_type"
            ]
        elif (
            self.ckpt_config["model"]["image_encoder"]["name"] == "resnet101"
            or self.ckpt_config["model"]["image_encoder"]["name"] == "resnet152"
            or self.ckpt_config["model"]["image_encoder"]["name"]
            == "tf_efficientnet_b5_ns-detect"
            or self.ckpt_config["model"]["image_encoder"]["name"]
            == "tf_efficientnetv2-detect"
        ):
        ...
        self.datamodule = DataModule(
            ...
            tokenizer_config=(
                self.ckpt_config["tokenizer"]
                if "tokenizer" in self.ckpt_config
                else None
            ),
            transform_config=(
                self.config["transform"]
                if "transform" in self.config
                else self.ckpt_config["transform"]
            ),
    ...
    def get_embeddings(self, checkpoint, test_dataset_name):
        ...
        for batch in tqdm(dataloader):
            if (
                self.clip_image_encoder == "swin"
                or self.clip_image_encoder == "resnet101"
                or self.clip_image_encoder == "resnet152"
                or self.clip_image_encoder == "tf_efficientnet_b5_ns-detect"
                or self.clip_image_encoder == "tf_efficientnetv2-detect"
            ):
    ...
    def encode_image(self, image: torch.Tensor):
        with torch.no_grad():
            ...
            img_emb = (
                self.model.image_projection(img_emb)
    ...
    def encode_text(self, text_token: Union[str, List[str], Dict, torch.Tensor]):
        ...
        with torch.no_grad():
            ...
            text_emb = (
                self.model.text_projection(text_emb)
                if self.model.projection
                else text_emb
            )
    ...
    def eval_zeroshot(self, checkpoint, test_dataset_name, zs_prompts, save_path):
        ...
        torch.save(emb, os.path.join(save_path, "embeddings.pth"))
        results = {}
        log.info(zs_prompts)
        for label_text in zs_prompts:
            ...
            if label_text.lower() == "suspicious_calcification":
                ...
                fpr, tpr, thresholds = metrics.roc_curve(
                    emb["cancer"], similarities[:, 1]
                )
                auroc = metrics.auc(fpr, tpr)
                results[label_text] = auroc

        print("Dataset", test_dataset_name)
    ...
    def eval_img_text_retrieval(self, checkpoint, test_dataset_name, save_path):
        ...
        similarities = metrics.pairwise.cosine_similarity(
            image_embeddings, identical_text_embedding
        )  # n x m
        ...
        log.info(
            "\n".join(
                [f"Recall@{k}: {v / num_samples:.3f}" for k, v in recall_dict.items()]
            )
...
def multiclass_classification(preds: np.ndarray, labels: np.ndarray, class_list: list):
    ...
    class_dict = {
        class_name: {"total_num": 0, "correct_num": 0} for class_name in class_list
    }
    for idx, class_name in enumerate(class_list):
        class_dict[class_name]["total_num"] = labels[:, idx].sum()
        class_dict[class_name]["correct_num"] = (
            labels[:, idx] * (preds_args == idx)
        ).sum()
    ...
    result["Accuracy(Micro)"] = (
        correct_num / total_num
    )  # same with macro due to same total_num
