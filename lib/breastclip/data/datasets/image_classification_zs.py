...
class ImageClassificationZSDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        log.info(f"Applying mean: {mean} and std: {std} to images")
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset.lower() == "vindr":
            ...
            return {"images": img, "mass": mass, "calc": calc, "density": density}
        elif self.dataset.lower() == "rsna":
            cancer = torch.tensor(data["cancer"], dtype=torch.long)
            return {"images": img, "malignancy": cancer, "cancer": cancer}
