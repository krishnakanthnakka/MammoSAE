...
class ImageAligenerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pred = torch.tensor(
            data["predictions_aucroc_weighted_BCE_y_bal_dataloader_n"], dtype=torch.long
        )
        age = torch.tensor(data["age"], dtype=torch.long)
        calc_0_1 = torch.tensor(
            data["Suspicious_Calcification_th_0.1"], dtype=torch.long
        )
        calc_0_15 = torch.tensor(
            data["Suspicious_Calcification_th_0.15"], dtype=torch.long
        )
        calc_0_25 = torch.tensor(
            data["Suspicious_Calcification_th_0.25"], dtype=torch.long
        )
