...
class ImageTextDataset(Dataset):
    ...
    def __getitem__(self, index):
            ...
            if len(view_list) >= 2:
                ...
                if self.split == "train":
                    if self.image_aug_other_image and len(image_path_list) > 1:
                        image_path_list = np.random.choice(
                            image_path_list, size=2, replace=False
                        )
                    else:
        ...
        if (
            self.image_encoder_type == "swin"
            or self.image_encoder_type == "resnet101"
            or self.image_encoder_type == "resnet152"
            or self.image_encoder_type == "tf_efficientnet_b5_ns-detect"
            or self.image_encoder_type == "tf_efficientnetv2-detect"
        ):
        ...
        if self.image_view_aug:
            ...
            image_view = torch.tensor(
                (image_view - self.mean) / self.std, dtype=torch.float32
            )
        ...
        if hasattr(self.df, "text"):
            ...
            if len(text_list) >= 2:
                ...
                text = (
                    text_aug_list[indexes[0]]
                    if random.random() < 0.5 and self.has_backtranslated
                    else text_list[indexes[0]]
                )
                text2 = (
                    text_aug_list[indexes[1]]
                    if random.random() < 0.5 and self.has_backtranslated
                    else text_list[indexes[1]]
                )
                ...
                if random.random() < 0.5:
                    text = text_list[0]
                    text2 = (
                        text_aug_list[0] if self.has_backtranslated else text_list[0]
                    )
                else:
        ...
        # Get from image-label dataset.
            ...
            if self.dataset.lower() == "vindr":
                ...
                text = generate_report_from_labels(
                    cc_findings, self.prompt_json, deterministic=(self.split != "train")
                )
