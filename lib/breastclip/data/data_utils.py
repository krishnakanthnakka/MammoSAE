...
def load_transform(split: str = "train", transform_config: Dict = None):
    ...
    if split == "train":
        if (
            transforms["Resize"]["size_h"] == 512
            or transforms["Resize"]["size_h"] == 224
        ) and (
            transforms["Resize"]["size_w"] == 512
            or transforms["Resize"]["size_w"] == 224
        ):
            return Compose(
                [
                    Resize(
                        width=transforms["Resize"]["size_h"],
                        height=transforms["Resize"]["size_w"],
                    ),
                    HorizontalFlip(),
                    VerticalFlip(),
                    Affine(
                        rotate=transforms["transform"]["affine_transform_degree"],
                        translate_percent=transforms["transform"][
        ...
        if (
            transforms["Resize"]["size_h"] == 512
            and transforms["Resize"]["size_w"] == 512
        ):
            return Compose(
                [
                    Resize(
