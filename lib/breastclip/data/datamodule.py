...
class DataModule:
    def __init__(
        ...
        self.tokenizer = (
            load_tokenizer(**self.tokenizer_config)
        ...
        for dataset in data_config:
            df = pd.read_csv(
                Path(data_config[dataset]["data_dir"])
                / data_config[dataset]["data_path"],
                dtype=dtype_options,
            )
            df = df.fillna(0)
            ...
            train_dataset = load_dataset(
                ...
                label_col=(
                    data_config[dataset]["label_col"]
                    if "label_col" in data_config[dataset]
                    else None
                ),
                label_text=(
                    data_config[dataset]["label_text"]
                    if "label_text" in data_config[dataset]
                    else None
                ),
            )
        ...
        if self.train_loader is None:
            ...
            if distributed:
                self.dataloader_config["train"][
                    "shuffle"
                ] = False  # Disable shuffle if using DistributedSampler

                # Create DistributedSampler
                self.train_sampler = DistributedSampler(
                    dataset=dataset,
                    num_replicas=util.GlobalEnv.get().world_size,
