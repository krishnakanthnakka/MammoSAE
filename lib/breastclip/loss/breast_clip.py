...
class BreastClip(nn.Module):
    def __init__(
        self, label_smoothing=0.0, i2i_weight=0.0, t2t_weight=0.0, loss_ratio=1.0
    ):
    ...
    def forward(
        ...
        **kwargs
        ...
        loss_i2t += F.cross_entropy(
            logits_per_image, labels, label_smoothing=label_smoothing
        )
        loss_t2i += F.cross_entropy(
            logits_per_text, labels, label_smoothing=label_smoothing
        )
        ...
        if is_train:
            util.GlobalEnv.get().summary_writer.train.add_scalar(
                "loss/contrastive/steps_i2t",
                loss_i2t,
                util.GlobalEnv.get().summary_writer.global_step,
            )
            util.GlobalEnv.get().summary_writer.train.add_scalar(
                "loss/contrastive/steps_t2i",
                loss_t2i,
                util.GlobalEnv.get().summary_writer.global_step,
            )
            util.GlobalEnv.get().summary_writer.train.add_scalar(
                "loss/contrastive/steps_i2i",
                loss_i2i,
                util.GlobalEnv.get().summary_writer.global_step,
            )
            util.GlobalEnv.get().summary_writer.train.add_scalar(
                "loss/contrastive/steps_t2t",
                loss_t2t,
                util.GlobalEnv.get().summary_writer.global_step,
            )
            util.GlobalEnv.get().summary_writer.train.add_scalar(
