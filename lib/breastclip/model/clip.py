...
class BreastClip(nn.Module):
    def __init__(
        ...
        self.text_encoder = load_text_encoder(
            model_config["text_encoder"], vocab_size=tokenizer.vocab_size
        )
        ...
        self.temperature = (
            model_config["temperature"] if "temperature" in model_config else None
        )
        if self.temperature:
    ...
    def forward(self, batch, device=None):
        ...
        image_embeddings = (
            self.image_projection(image_features_g)
            if self.projection
            else image_features_g
        )
        text_embeddings = (
            self.text_projection(text_features_g)
            if self.projection
            else text_features_g
        )
        ...
        if "text_tokens2" in batch and "image_views" in batch:
            ...
            text_embeddings2 = (
                self.text_projection(text_features_g2)
                if self.projection
                else text_features_g
            )
            text_embeddings2 = text_embeddings2 / text_embeddings2.norm(
                dim=1, keepdim=True
            )
            out["text_embeddings2"] = text_embeddings2

            image_view_encode = self.encode_image(batch["image_views"].to(device))
            image_view_embeddings = (
                self.image_projection(image_view_encode)
                if self.projection
                else image_view_encode
            )
            image_view_embeddings = image_view_embeddings / image_view_embeddings.norm(
                dim=1, keepdim=True
            )
