    ...
    def __init__(self, model_config: Dict, model_type: str = "vit"):
        ...
        if model_config["load_backbone_weights"] is None:
            ...
            if not os.path.isfile(model_config["load_backbone_weights"]):
                raise ValueError(
                    f"Cannot find a weight file: {model_config['load_backbone_weights']}"
                )
        ...
        self.classifier = load_image_classifier(
            model_config["classifier"]["config"], self.image_encoder.out_dim
        )
